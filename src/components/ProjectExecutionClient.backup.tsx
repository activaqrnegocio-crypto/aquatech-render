...
export default function ProjectExecutionClient({ 
  ...
  useEffect(() => {
    ...
    const handleSyncSuccess = (e: any) => {
      if (e.detail?.projectId === idFromUrl) {
        // v373: Removed revalidateRoute — debounced fetchMessages below already handles refresh
  ...
  const handleSaveTeam = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setLocalTeam(newTeam)
        setIsEditingTeam(false)
        // v373: Removed revalidateRoute — setLocalTeam already updated state
      } else {
      ...
      const processExpense = async () => {
        try {
          ...
          try {
            ...
            if (res.ok) {
              // v373: Removed revalidateRoute — local expenses already updated
            }
          } catch (err) {
            await db.transaction('rw', db.outbox, async () => {
              await db.outbox.add({
  ...
  const handleDeleteExpense = async (expenseId: number) => {
    ...
    try {
      ...
      if (res.ok) {
        setLocalExpenses(prev => prev.filter(e => e.id !== expenseId))
        // v373: Removed revalidateRoute — local expenses already filtered
      }
    } catch (error) {
