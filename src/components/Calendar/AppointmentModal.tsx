...
export default function AppointmentModal({
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      if (initialData) {
        // v500: Support assignedUsers for multi-operator tasks
        let operatorIds: number[] = []
        if (initialData.assignedUsers) {
          const parsed = typeof initialData.assignedUsers === 'string' ? JSON.parse(initialData.assignedUsers) : initialData.assignedUsers
          operatorIds = parsed.map((u: any) => u.id)
        } else if (initialData.userId) {
          operatorIds = [initialData.userId]
        }
                ...
                {isAdminView && (
                  <div className="form-group-compact">
                    ...
                    <div className="operator-dropdown-wrapper">
                      <div className="operator-dropdown-trigger" onClick={() => setIsDropdownOpen(!isDropdownOpen)}>
                        {selectedOperatorIds.length === 0
                          ? 'Seleccionar operador...'
                          : selectedOperatorIds.length === 1
                            ? operators.find(op => op.id === selectedOperatorIds[0])?.name || '1 seleccionado'
                            : `${selectedOperatorIds.length} seleccionados`
                        }
                      </div>
                      {isDropdownOpen && (
